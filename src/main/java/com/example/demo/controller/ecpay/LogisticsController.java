...
// http://localhost:8080/contextPath/hello/abc/def?qty=1&type=student
	...
	public String index() {
		...
		return "ecpay/test";    //  /WEB-INF/views/index.jsp
	}
...
//				obj.setServerReplyURL("https://f4e6-123-110-45-107.ngrok-free.app/end/MyTest/oni/reciever.java");
				...
				obj.setServerReplyURL("https://6ce2-111-253-207-93.ngrok-free.app/logistics/StoreReceiver");
...
//				response.getWriter().append(store).append(request.getContextPath());
		...
		return store;    //  /WEB-INF/views/index.jsp
	
	}
	@PostMapping("/StoreReceiver")
//	@ResponseBody
	public String StoreReceiver( Model model,HttpServletRequest request,HttpServletResponse  response,RedirectAttributes redirectAttributes) {
...
//		System.out.println("測試內容"+request.getParameter("CVSStoreName"));
		redirectAttributes.addFlashAttribute("CVSStoreName", request.getParameter("CVSStoreName"));
		redirectAttributes.addFlashAttribute("CVSAddress", request.getParameter("CVSAddress"));
		redirectAttributes.addFlashAttribute("CVSStoreID", request.getParameter("CVSStoreID"));

		//		redirectAttributes.addAllAttributes(Collections.list(request.getParameterNames()));
		return "redirect:/checkout";
//		不可以直接返回到 jsp 掠過 controller 因為會直接GG 
//		(詳細原因估計是 相對路徑 混亂 (依賴controller轉發路徑判斷!
	}
