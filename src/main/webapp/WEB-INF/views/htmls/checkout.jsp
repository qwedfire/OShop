...
<%--     <c:set var="prodPrice" value="${item.price}" /> --%>
<%--   </c:if> --%>
<%--   	</c:forEach> --%>
<%--   	<c:forEach items="${rs3.rows}" var="item"> --%>
<%--   <c:if test="${item.prodID eq productID}"> --%>
...
<%--   </c:if> --%>
<%-- </c:forEach> --%>
<%--   	<c:set var="totalCost" value="${totalCost + row.quantity * prodPrice}" /> --%>
<%-- </c:forEach> --%>
...
<%-- 			    url="jdbc:mysql://localhost:3306/os_shop" user="root" password="root"/> --%>
			
		<sql:query var="result">
		    SELECT price FROM os_product WHERE id = ${row.productID}
		</sql:query>
		<c:set var="prodPrice" value="${result.rows[0].price}" />
	    <c:set var="totalCost" value="${totalCost + row.quantity * prodPrice}" />
			...
			<div class="row">
				...
				<div class="col-md-4 col-xs-12 col-sm-4">
					...
					<div class="logo text-center">
						...
						<!-- *** LOGO連結 回到首頁 *** -->
						<a href="<c:url value='/index'/>"> <!-- 頁首標題欄 --> <svg
				...
				<div class="col-md-4 col-xs-12 col-sm-4">
					...
					<ul class="top-menu text-right list-inline">
						...
						<li class="dropdown search dropdown-slide"><a
							...
							<ul class="dropdown-menu search-dropdown">
								<li><a style="font-size: 16px"
									href="<c:url value='/signin'/>">註冊</a></li>
								<li><a style="font-size: 16px"
						...
						<li class="dropdown cart-nav dropdown-slide">
							...
							data-hover="dropdown"><i class="tf-ion-android-cart"
								style="font-size: 23px"></i></a>
							<div class="dropdown-menu cart-dropdown">

								<!-- 購物車內容1 -->
								<div class="media">
									<a href="<c:url value=''/>" class="pull-left"> <img
										class="media-object" src="images/oShopBlack-no.png"
										alt="image" />
									</a>
								...
								<div class="cart-summary">
									<span>合計</span> <span class="total-price">NT$1799</span>
								</div>
						...
						<li class="dropdown search dropdown-slide">
							...
							class="dropdown-toggle" data-toggle="dropdown"
							data-hover="dropdown"><i class="tf-ion-ios-search-strong"
								style="font-size: 23px"></i> </a>
							<ul class="dropdown-menu search-dropdown">
								<li>
									<form action="post">
					...
					<ul class="nav navbar-nav">
						...
						<li class="dropdown dropdown-slide"><a
							...
							role="button" aria-haspopup="true" aria-expanded="false">桌上型電腦<span
								class=""></span></a> <!-- class="tf-ion-ios-arrow-down" --> <!-- 							<div class="dropdown-menu"> -->
							<!-- 								<div class="row"> --> <!-- 									*** 筆電 導覽列 有跳轉頁面 *** -->
							<!-- 									<div class="col-lg-6 col-md-6 mb-sm-3"> --> <!-- 										<ul> -->
							<!-- 											<li class="dropdown-header">依系列&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</li> -->
							<!-- 											<li role="separator" class="divider"></li> --> <!-- 											<li><a href="<c:url value='/product-single'/>">Swift</a></li> --!>
<!-- 											<li><a href="<c:url value='/product-single'/>">Aspire</a></li> --!>
<!-- 											<li><a href="<c:url value='/product-single'/>">其他系列</a></li> --!>
<!-- 										</ul> --> <!-- 									</div> --> <!-- 									*** 周邊 導覽列 有跳轉頁面 *** -->
							<!-- 									<div class="col-lg-6 col-md-6 mb-sm-3"> --> <!-- 										<ul> -->
							<!-- 											<li class="dropdown-header">電腦周邊&emsp;</li> --> <!-- 											<li role="separator" class="divider"></li> -->
							...
							<div class="checkout-product-details">
								<div class="payment">
									<div class="card-details">
										...
										<!-- 付款方式 END-->
										<div>
											<p style="font-size: 14px">⚠ 下單後請盡速付款，如超過繳款期限，系統將自動取消訂單。</p>
											<p style="font-size: 14px">⚠【收件人姓名請輸入’’真實姓名’’，超商取貨時需核對證件。】</p>
											<p style="font-size: 14px">⚠ 全面使用【雲端電子發票】電子發票證明聯系統會寄至買家E-mail</p>
										</div>
									</div>
					...
					<div class="col-md-5">
						<div class="product-checkout-details">
							<div class="block">
								...
								<!--                       <img -->
								<!--                         class="media-object" -->
								<!--                         src="images/oshop.jpeg" -->
								<!--                         alt="Image" -->
								<!--                       /> -->
								<!--                     </a> -->
								<!--                     <div class="media-body"> -->
								<!--                       <h4 class="media-heading"> -->
								<!--                         <a class="product-single-new" href="product-single.html" style="font-size: 18px" -->
								<!--                           >AAA筆記型電腦</a -->
								<!--                         > -->
								<!--                       </h4> -->
								<!--                       <p class="price" style="font-size: 18px">1 x NT$249</p> -->
								<!--                       <span class="remove" >移除</span> -->
								<!--                       <a -->
								<!--                         id="trash" -->
								<!--                         class="tf-ion-ios-trash" -->
								...
								<ul class="summary-prices">
									<li><span style="font-size: 16px">小計:</span> <span
										class="price" style="font-size: 16px" id="money1">${totalCost }</span></li>
								...
								<div class="summary-total">
									...
									<span id="money"
										style="font-size: 16px">${totalCost }</span>
								</div>
								...
								<div class="product-checkout-711">
									<div class="block">
										...
										<section class="delivery-form"
											...
											<div class="section-body">
												<p style="font-size: 16px">已選擇的送貨方式: 全家 純取貨，不付款 (先進行付款，超商直接取貨)</p>
												...
												<div id="delivery-form-content">
													<div class="form-group" ng-if="isCreatingNewAddress()">
														<label class="control-label"> <input
															...
															name="order[delivery_data][recipient_is_customer]" style="font-size: 16px">
															收件人資料與顧客資料相同
														</label>
													</div>
													<div class="form-group">
														<label for="recipient-name" class="control-label" style="font-size: 16px">收件人名稱</label>
														<input id="recipient-name" type="text"
															...
															required style="font-size: 16px"/> <span class="help-block" style="font-size: 16px">請填入收件人真實姓名，以確保順利收件</span>
													</div>


													<div class="form-group intl-tel-input-container">
														<label for="recipient-phone" class="control-label" style="font-size: 16px">收件人電話號碼</label>
													...
													<div id="family-mart-address" checkout-delivery-family-mart
														...
														<div class="form-group">
															<label for="order-delivery-location-code"
																...
																src="https://static.shoplineapp.com/web/assets/services/family_mart.jpg" style="font-size: 18px">
																<c:choose>
															...
															<br>
															<br>
															<div id="searchstore" class="btn btn-success btn-block btn-pick-store" style="font-size: 16px">
																搜尋門市
...
<!-- 						</li> -->
						<!-- 					</ul> -->
						<p class="copyright-text" style="font-size: 12px;">
							版權 &copy;2021, 設計 &amp; 開發 <a href="https://themefisher.com/">Themefisher</a>
						</p>
					</div>
				</div>
			</div>
...
<!-- 		格式化金钱格式 -->
		<script>
		    ...
		    const money = Number(moneySpan.innerText);
...
<!-- 		訂單結帳>>>>>綠界   開啟新分頁的寫法 -->
		...
		<script >
			function openPaymentPage(){
				let paymentUrl = '<c:url value="/pay"/>';
		...
		<script >
			...
			$(function(){
				...
				function triggerLogisticJSP() {
					...
					$.ajax({
						type:"GET",
						url:"logistics/ChooseStore",
// 						url:"ChooseStore",
						data:[]
					}).then(function(e){
						console.log(e)
	//						$("#dd").html(e);		
						$("#logistic_ecpay").html(e);	
						console.log("找門市!?");
						
					})
				}
// 				function triggerLogisticJSP(){
// 					let logisticUrl='<c:url value="/ChooseStore"/>';
// 					window.open(logisticUrl);
// 				}
				$("#searchstore").on("click",triggerLogisticJSP);
			})
		</script>
		
<script>
function submitOrder() {
  window.open('<c:url value="/pay"/>', '_blank');
  // 執行清空購物車的動作
	$.ajax({
	    type: 'GET',
	    url: 'clearCart',
	    ...
	    success: function(data) {
	    	...
	    	document.getElementById("money1").innerHTML = 0;
	      console.log(1);
	    },
	    error: function() {
	      console.log(2);
